			...
			initPlay(id,index) {
				...
				apiSong(id).then(res=>{
					...
					this.song = {
						...
						picUrl: res.pic_url,
						singer,
						time: Math.floor(res.dt / 10) // 播放时长
					}
					this.setPlaydetail({id,pic:res.pic_url})
					this.$au_player.url = this.song.url;
					this.$au_player.title = this.song.name;
					this.$au_player.coverImgUrl = this.song.pic_url;
