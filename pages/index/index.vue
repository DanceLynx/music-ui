<template>
	<view class="content">
		<bar @handle="barEvent" placeHolderVal="搜索歌曲" searchType="songs"></bar>
		...
		<view class="banners">
			<swiper class="swiper" :indicator-dots="true" :autoplay="true" :interval="2000" :duration="600" :circular="true">
				<swiper-item v-for="(ele,index) in banners" :key="index" @click="toMusic({id:ele.target_id,type:ele.target_type})">
					<view class="swiper-item">
						<image :src="ele.image_url" mode="" class="img"></image>
					</view>
				</swiper-item>
			</swiper>
		</view>
...
<script>
	...
	export default {
		...
		computed: {
			songlist1() {
				return this.songRecommendList;
			},
			songlist2() {
				return this.songHotList;
			},
		},
		components: {
			songList,
		...
		methods: {
			...
			getRecommendSongs() {
				getSongs('?limit=6&filter[is_recommend]=1').then(res => {
					this.songRecommendList = res.data;
				})
			},
			getHotSongs() {
				getSongs('?limit=6&filter[is_hot]=1').then(res => {
					this.songHotList = res.data;
				})
			},
			// getMvs(){
